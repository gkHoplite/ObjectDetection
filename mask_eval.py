...
def parse_rec(filename):
    ...
    object_list = []
    parsed_xml = cElementTree.parse(filename)
...
def mask_ap(rec, prec, use_07_metric=False):
...
def mask_eval(classname,
    ...
    ap = mask_ap(rec, prec, use_07_metric)

    return rec, prec, ap


def _write_mask_results_file(all_boxes, image_index, root, classes):
    if os.path.exists('/tmp/mask/results'):
        shutil.rmtree('/tmp/mask/results')
    os.makedirs('/tmp/mask/results')
...
def _do_mask_python_eval(data_loader):
