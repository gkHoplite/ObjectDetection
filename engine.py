...
from mask_eval import _write_mask_results_file, _do_mask_python_eval
import utils
...
@torch.no_grad()
def ekdark_evaluate(model, data_loader, device):
    n_threads = torch.get_num_threads()
    torch.set_num_threads(1)
    cpu_device = torch.device("cpu")
    model.eval()
...
@torch.no_grad()
def mask_evaluate(model, data_loader, device):
    n_threads = torch.get_num_threads()
    torch.set_num_threads(1)
    cpu_device = torch.device("cpu")
    model.eval()
    ...
    if utils.is_main_process():
        ...
        for iig in image_index_gathered:
            image_index += iig
        _write_mask_results_file(all_boxes, image_index, data_loader.dataset.root,
                                data_loader.dataset._transforms.transforms[0].CLASSES)
